...
- "skills" — two directories, two unrelated meanings; **resolved**, though not by the fix this entry used to propose. `agent_skills/` holds runtime **Skills** the assistant loads (ADR-0022). The root `skills/` holds development skills for people working *on* OpsPilot (grilling, tdd, to-spec …) and has nothing to do with the product. Renaming the root directory was never available: `skills/` is a symlink farm a third-party skill installer generates over `.agents/skills/`, alongside a second farm at `.claude/skills/` — the name is not ours, and deleting it does not survive the next sync. The rename that settles this was already made on the side we *do* own, and is the answer to "why `agent_skills/` and not `skills/`". All three farms are gitignored, so a clone has no root `skills/` at all, and `git grep`, `rg` and `grep -r` all miss it — the first two honour `.gitignore`, the third does not follow symlinked directories. A bare `ls` at the root is the only place both names still appear. The residual trap is the opposite of a rename: `.gitignore`'s root-anchored `/skills/` would silently swallow a tracked root `skills/`, so never add one — `agent_skills/` is the product's directory.
...
- "memory" — two meanings, one of them wrong; **resolved** (#167). The package holding the **KB** implementation (chunker, ingestion, retrieval, conflict) was `src/opspilot/memory/` and is now `src/opspilot/kb/`, so the word is free for **Memory**, the environment-constraint store defined above. Same shape as the `skills/` vs `agent_skills/` collision below it — but only in shape: that one was never ours to rename, and settled differently. Note `docs/specs/memory/` keeps its name deliberately; unlike the package, it genuinely holds both KB schemas and a `memory-record` schema, so it is a separate question.
